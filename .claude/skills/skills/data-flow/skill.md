...
读取 `开发文档/06-架构/shared-schema.ts`，将本页面的数据流结果扩展进去：

| 扩展内容 | 来源 | 写入位置 |
|---------|------|---------|
| API端点 | 第3步API契约表 | §4 API端点常量 |
| 跨页面参数 | 跨页面关联分析 | §6 跨页面参数 |
| 新增枚举 | 第2步字段溯源中出现的新枚举 | §1 枚举定义 |

**规则**：
- 已存在于 schema 中的枚举/端点不允许重复定义，只引用
- 新增端点必须使用 `as const` 格式，与已有端点风格一致
- 如发现 schema 中的命名与数据流设计冲突，以 schema 为准并调整数据流

输出确认：
```
✅ 数据流已生成：开发文档/04-数据流/{页面名}-数据流.md
📊 覆盖：字段溯源(📥🔗📊⛔) / API契约 / 前端状态 / 状态流转 / 系统影响
📦 shared-schema.ts 已扩展：+{N}个API端点 / +{M}个跨页面参数
...
5. **命名必须与 shared-schema.ts 一致**，不允许自行创造新字段名（遗漏的先注册到 schema 再使用）
