...
从全局模板复制**全部**技能卡到新项目的 `.claude/skills/`：

```bash
# 执行命令（复制所有技能卡，不限定数量）
mkdir -p .claude/skills
cp -rn ~/.claude/project-template/skills/* .claude/skills/
```

**规则**：
- 复制模板中**所有**技能卡文件夹（不限定数量，有多少复制多少）
- `-rn` 表示不覆盖已有文件，只添加不存在的
- 当前模板约95个技能，包含自建技能 + 通用技能
...
| 06 | 架构/ | — | architecture、database、API规范 |
| 07 | 契约/ | — | 冻结的业务契约 |
| 08 | 日志/ | — | 开发日志 |
| 09 | 归档/ | — | 历史文件 |
...
### 已创建
├── CLAUDE.md                项目协作指南
├── .claude/skills/ (~95个)  全部技能卡
├── 开发文档/ (10个文件夹)    文档结构
├── 03-视觉/shejigao.pen     设计系统（59个变量）
└── 03-视觉/设计系统规范.md    设计系统文档

### 完整链路
/refine → /design-spec → /design-detail → /design-page → /data-flow → /tech-spec → /align → /plan-first → 编码 → 验证 → 部署
