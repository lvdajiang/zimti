...
```bash
# catch 块中缺少用户反馈
grep -A2 "catch" packages/client/src/ --include="*.vue" --include="*.ts" | grep "console.error"
# 后端：裸 except
grep -rn "except:" backend/app/ --include="*.py"
```

**通过条件**：
- ✅ 所有 `catch` 块都有用户提示（`toast.error` / `toast.warning` / `alert`）
- ✅ 后端无裸 `except:`
...
```bash
# 只有 success 提示但没有 API 调用的函数
grep -B5 "toast.success\|alert(" packages/client/src/ --include="*.vue" | grep -v "await\|fetch\|axios\|api\."
# localStorage 做持久化（应该用后端API）
grep -rn "localStorage" packages/client/src/ --include="*.vue" --include="*.ts"
# 桩端点：返回硬编码 pending 但没有 markStub 标记
grep -rn "status: 'pending'" packages/server/src/routes/ --include="*.ts" | grep -v "markStub"
```

**通过条件**：
- ✅ 每个 `toast.success` / `alert` 前都有实际的 API 调用
- ✅ 关键数据不依赖 localStorage 持久化
- ✅ 桩端点（返回硬编码假数据）有 `markStub()` 调用或 `X-Stub` 标记

#### 2.7 编码模式扫描

```bash
# 裸 Number() 转换路由参数/查询参数
grep -rn "Number(req\.\(params\|query\)\." packages/server/src/routes/ --include="*.ts"
# 前端 Number() 转换路由参数
grep -rn "Number(route.params\." packages/client/src/ --include="*.vue" --include="*.ts"
```

**通过条件**：
- ✅ `Number()` 转换外部输入时使用了 `toInt()` 或有 `NaN` 检查
- ✅ 列表组件有空状态处理（v-if/v-else 或 empty slot）
...
```markdown
## [日期 时间] /code-review [审查范围]
| # | 严重程度 | 类别 | 文件:行号 | 问题描述 |
|---|---------|------|----------|---------|
| 1 | ❌高 | 性能 | xxx:L42 | pageSize=1000 |
| 2 | ⚠️中 | 错误处理 | xxx:L63 | catch无用户提示 |
```
...
- 审查后问题自动累积到 `.claude/review-issues.log`，累积 ≥ 3 个时提示运行 `/retro`
