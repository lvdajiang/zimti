...
- **决策**: 接入 GPT Image 2.0 作为第三图片来源
  - **原因**: 用户确认；OpenAI API Key 已有（Whisper 共用）；文字理解能力比即梦强
  - **影响**: 后端新增 OpenAI Images service，前端素材库/选题工作台加入口，与即梦并列
  - **成本**: 约 $0.04-0.08/张，比即梦贵
  - **替代方案**: 只用即梦（更便宜但文字配合能力弱）

- **决策**: AI 视频制作采用「GLM 拆分镜 + AI 生画面 + Remotion 组装」路线
  - **原因**: 不需要自研视频生成模型，复用已有即梦/GPT Image 能力；Remotion 已有渲染基础
  - **影响**: 在现有脚本→制作→渲染流程中插入 AI 分镜步骤
  - **替代方案**: 全抄 Flova 模式（做独立编辑器，偏离管理系统定位）
