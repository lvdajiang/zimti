...
### 已创建
├── CLAUDE.md              项目协作指南
├── .claude/skills/ (24)   核心技能卡
├── 开发文档/ (10个文件夹)  文档结构
├── 03-视觉/shejigao.pen   设计系统（59个变量）
└── 03-视觉/设计系统规范.md  设计系统文档

### 完整链路
/refine → /init-schema → /design-spec → /design-detail → /design-page → /data-flow → /tech-spec → /align → /plan-first → 编码 → 验证 → 部署
             ↓ 锚定命名                                              ↓ 扩展API      ↓ 扩展表结构
...
├── CLAUDE.md              项目协作指南模板
├── skills/ (24个)         核心技能卡（含 init-schema）
├── memory/ (5个)          记忆文件模板
└── 开发文档/ (10个文件夹)  文档结构模板
```
...
**通用 vs 项目特有判断**：
- 通用：`/refine` `/init-schema` `/design-*` `/data-flow` `/tech-spec` `/align` `/plan-*` `/check-page` `/code-review` `/component-check` `/perf-check` `/deploy-check` `/regression` `/xgbug` `/code-fix` `/status` `/git-clean` `/save-ctx` `/load-ctx` `/db` `/brainstorming`
