...
const overview = ref<{ recent_publishes_7d: number; total_plays: number; avg_completion_rate: number } | null>(null)
const stats = ref<{ total_tasks: number; active_tasks: number; total_videos: number; published_videos: number; total_materials: number; total_assets: number } | null>(null)
...
async function loadVideoRecords(): Promise<void> {
  try {
    const res = await api.get<{ items: typeof videoRecords.value }>('/dashboard/video-records?page_size=50')
