...
function initAIProvider(): void {
  ...
  if (apiKey && baseURL) {
    ...
    logger.info(`[AI] GLM Provider initialized (model: ${model ?? 'default'})`)
  } else {
    logger.warn('[AI] GLM_API_KEY/GLM_BASE_URL not set, using MockAIProvider')
  }
}
...
async function ensureDemoUser(): Promise<void> {
  ...
  if (!existing) {
    ...
    logger.info('[DB] Created demo user')
  }
}
