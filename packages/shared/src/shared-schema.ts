...
export const API = {
  ...
  // --- 仪表盘 ---
  DASHBOARD: {
    WORKFLOW: '/api/v1/dashboard/workflow',
    STATS: '/api/v1/dashboard/stats',
    OVERVIEW: '/api/v1/dashboard/overview',
    VIDEO_RECORDS: '/api/v1/dashboard/video-records',
    TRENDS: '/api/v1/dashboard/trends',
    SNAPSHOTS: '/api/v1/dashboard/snapshots',
    AI_ANALYSIS: '/api/v1/dashboard/ai-analysis',
    AI_ANALYSIS_STATUS: (taskId: string) => `/api/v1/dashboard/ai-analysis/${taskId}`,
  },
  ...
  TASKS: {
    LIST: '/api/v1/tasks',
    CREATE: '/api/v1/tasks',
    CONVERSION_GOAL: (taskId: string) => `/api/v1/tasks/${taskId}/conversion-goal`,
    IMPORT_INSTRUCTIONS: (taskId: string) => `/api/v1/tasks/${taskId}/import-instructions`,
    IMPORT_EXPERIENCE: (taskId: string) => `/api/v1/tasks/${taskId}/import-experience`,
  },

  // --- 对标账号 ---
  BENCHMARK_ACCOUNTS: {
    LIST: '/api/v1/benchmark-accounts',
    STATS: '/api/v1/benchmark-accounts/stats',
    CREATE: '/api/v1/benchmark-accounts',
    UPDATE: (id: string) => `/api/v1/benchmark-accounts/${id}`,
    DELETE: (id: string) => `/api/v1/benchmark-accounts/${id}`,
    TOGGLE_MONITOR: (id: string) => `/api/v1/benchmark-accounts/${id}/monitor`,
  },

  // --- 采集任务 ---
  COLLECT_TASKS: {
    LIST: '/api/v1/collect-tasks',
    STATS: '/api/v1/collect-tasks/stats',
    CREATE: '/api/v1/collect-tasks',
    UPDATE: (id: string) => `/api/v1/collect-tasks/${id}`,
    DELETE: (id: string) => `/api/v1/collect-tasks/${id}`,
    BATCH_DELETE: '/api/v1/collect-tasks/batch',
    EXECUTE: (id: string) => `/api/v1/collect-tasks/${id}/execute`,
    PAUSE: (id: string) => `/api/v1/collect-tasks/${id}/pause`,
    RETRY: (id: string) => `/api/v1/collect-tasks/${id}/retry`,
    RERUN: (id: string) => `/api/v1/collect-tasks/${id}/rerun`,
    LOGS: (id: string) => `/api/v1/collect-tasks/${id}/logs`,
    SCHEDULE: '/api/v1/collect-tasks/schedule',
  },

  // --- 爆款视频 ---
  VIRAL_VIDEOS: {
    LIST: '/api/v1/viral-videos',
    DETAIL: (id: number) => `/api/v1/viral-videos/${id}`,
    ANALYZE_BATCH: '/api/v1/viral-videos/analyze-batch',
    EXPORT: '/api/v1/viral-videos/export',
    TRANSCRIPT_EXTRACT: (id: number) => `/api/v1/viral-videos/${id}/transcript/extract`,
    ANALYZE: (id: number) => `/api/v1/viral-videos/${id}/analyze`,
    SAVE_TRANSCRIPT: (id: number) => `/api/v1/viral-videos/${id}/transcript`,
    DOWNLOAD: (id: number) => `/api/v1/viral-videos/${id}/download`,
  },
  ...
  // --- 关键词监控 ---
  KEYWORD_MONITORS: {
    LIST: '/api/v1/keyword-monitors',
    TRENDS: '/api/v1/keyword-monitors/trends',
    CREATE: '/api/v1/keyword-monitors',
    DELETE: (id: number) => `/api/v1/keyword-monitors/${id}`,
  },

  // --- 选题提案 ---
  TOPIC_PROPOSALS: {
    GENERATE: '/api/v1/topic-proposals/generate',
    GENERATE_STATUS: (taskId: string) => `/api/v1/topic-proposals/generate/${taskId}/status`,
    SELECT: (topicId: number) => `/api/v1/topic-proposals/${topicId}/select`,
    MERGE: '/api/v1/topic-proposals/merge',
    ADD_VIDEOS: '/api/v1/topic-proposals/add-videos',
  },
  ...
  PERSONA: {
    GET: '/api/v1/persona',
    SAVE: '/api/v1/persona',
    UPLOAD_SAMPLE: '/api/v1/persona/samples',
    DELETE_SAMPLE: (id: number) => `/api/v1/persona/samples/${id}`,
    PREVIEW: '/api/v1/persona/preview',
  },

  // --- 脚本 ---
  SCRIPTS: {
    GET: (id: number) => `/api/v1/scripts/${id}`,
    CREATE: '/api/v1/scripts',
    SAVE: (id: number) => `/api/v1/scripts/${id}`,
    SEGMENTS: (id: number) => `/api/v1/scripts/${id}/segments`,
    GENERATE_STORYBOARD: (id: number) => `/api/v1/scripts/${id}/generate-storyboard`,
    STORYBOARD_STATUS: (scriptId: number, taskId: string) =>
      `/api/v1/scripts/${scriptId}/generate-storyboard/${taskId}/status`,
    SAVE_SEGMENT: (scriptId: number, segmentId: number) =>
      `/api/v1/scripts/${scriptId}/segments/${segmentId}`,
    ADD_SEGMENT: (id: number) => `/api/v1/scripts/${id}/segments`,
    DELETE_SEGMENT: (scriptId: number, segmentId: number) =>
      `/api/v1/scripts/${scriptId}/segments/${segmentId}`,
    REORDER_SEGMENTS: (id: number) => `/api/v1/scripts/${id}/segments/reorder`,
    AI_CHECK: (id: number) => `/api/v1/scripts/${id}/ai-check`,
    APPLY_SUGGESTION: (id: number) => `/api/v1/scripts/${id}/apply-suggestion`,
    GENERATE_VOICEOVER: (id: number) => `/api/v1/scripts/${id}/generate-voiceover`,
    VOICEOVER_STATUS: (scriptId: number, taskId: string) =>
      `/api/v1/scripts/${scriptId}/generate-voiceover/${taskId}/status`,
    PREVIEW_AUDIO: (id: number) => `/api/v1/scripts/${id}/preview-audio`,
    MATERIALS: (id: number) => `/api/v1/scripts/${id}/materials`,
    CONFIRM: (id: number) => `/api/v1/scripts/${id}/status`,
    STORYBOARD: (scriptId: string) => `/api/v1/scripts/${scriptId}/storyboard`,
    UPDATE_TRANSITION: (scriptId: string, segmentId: string) =>
      `/api/v1/scripts/${scriptId}/segments/${segmentId}/transition`,
    REPLACE_MATERIALS: (scriptId: string, segmentId: string) =>
      `/api/v1/scripts/${scriptId}/segments/${segmentId}/materials`,
    RENDER_CONFIG: (scriptId: string) => `/api/v1/scripts/${scriptId}/render-config`,
    SAVE_PROGRESS: (scriptId: string) => `/api/v1/scripts/${scriptId}/save-progress`,
    COMPOSITION: (scriptId: string) => `/api/v1/scripts/${scriptId}/composition`,
  },

  // --- 素材 ---
  MATERIALS: {
    LIST: '/api/v1/materials',
    STATS: '/api/v1/materials/stats',
    UPLOAD: '/api/v1/materials',
    GENERATE: '/api/v1/materials/generate',
    GENERATE_STATUS: (taskId: string) => `/api/v1/materials/generate/${taskId}/status`,
    GENERATE_CONFIRM: (taskId: string) => `/api/v1/materials/generate/${taskId}/confirm`,
    PEXELS_SEARCH: '/api/v1/materials/pexels-search',
    PEXELS_IMPORT: '/api/v1/materials/pexels-import',
    UPDATE: (id: string) => `/api/v1/materials/${id}`,
    DELETE: (id: string) => `/api/v1/materials/${id}`,
    REFERENCES: (id: string) => `/api/v1/materials/${id}/references`,
    DOWNLOAD: (id: string) => `/api/v1/materials/${id}/download`,
  },

  // --- 视频产品 ---
  VIDEO_PRODUCTS: {
    PUBLISH_WORKSPACE: (id: string) => `/api/v1/video-products/${id}/publish-workspace`,
    GENERATE_COPY_BATCH: (id: string) => `/api/v1/video-products/${id}/generate-copy/batch`,
    PREVIEW: (id: string) => `/api/v1/video-products/${id}/preview`,
    RENDER: '/api/v1/video-products/render',
    RENDER_STATUS: (id: string) => `/api/v1/video-products/${id}/render-status`,
    RENDER_CANCEL: (id: string) => `/api/v1/video-products/${id}/render-cancel`,
    DERIVE: (id: string) => `/api/v1/video-products/${id}/derive`,
  },

  // --- 发布记录 ---
  PUBLISH_RECORDS: {
    GET: '/api/v1/publish-records',
    GENERATE_COPY: (id: string) => `/api/v1/publish-records/${id}/generate-copy`,
    SAVE_CONTENT: (id: string) => `/api/v1/publish-records/${id}/content`,
    SEO_CHECK: (id: string) => `/api/v1/publish-records/${id}/seo-check`,
    AUTO_SAVE: (id: string) => `/api/v1/publish-records/${id}/auto-save`,
    ADD_TAG: (id: string) => `/api/v1/publish-records/${id}/tags`,
    REMOVE_TAG: (recordId: string, tag: string) => `/api/v1/publish-records/${recordId}/tags/${tag}`,
    GENERATE_GEO: (id: string) => `/api/v1/publish-records/${id}/generate-geo`,
    PUBLISH: (id: string) => `/api/v1/publish-records/${id}/publish`,
    AIGC_CONFIRM: (id: string) => `/api/v1/publish-records/${id}/aigc-confirm`,
    CONVERSION_TYPE: (id: string) => `/api/v1/publish-records/${id}/conversion-type`,
  },

  // --- 内容资产 ---
  CONTENT_ASSETS: {
    LIST: '/api/v1/content-assets',
    DETAIL: (id: string) => `/api/v1/content-assets/${id}`,
    MATERIALS: (id: string) => `/api/v1/content-assets/${id}/materials`,
    TRENDS: (id: string) => `/api/v1/content-assets/${id}/trends`,
    CREATE_HIGHLIGHT: (id: string) => `/api/v1/content-assets/${id}/highlights`,
    REUSE_MATERIALS: (id: string) => `/api/v1/content-assets/${id}/reuse-materials`,
    REUSE_SCRIPT: (id: string) => `/api/v1/content-assets/${id}/reuse-script`,
    DELETE: (id: string) => `/api/v1/content-assets/${id}`,
    EXPORT: (id: string) => `/api/v1/content-assets/${id}/export`,
    AUTO_CREATE: '/api/v1/content-assets/auto-create',
  },

  // --- 经验日志 ---
  EXPERIENCE_LOGS: {
    CREATE: '/api/v1/experience-logs',
  },
