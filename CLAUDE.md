...
### 行为规则类
`/refine` 需求细化(想法→需求) | `/init-schema` 建立共享定义(需求→命名锚点) | `/design-spec` 设计规格(需求→7要素文档) | `/design-detail` 页面描述(设计规格→逐页蓝图) | `/design-page` 视觉设计(页面描述→Pencil稿) | `/data-flow` 数据流设计(页面→API+扩展schema) | `/tech-spec` 技术规格(数据流→编码蓝图+扩展schema) | `/align` 对齐理解(7问+穷举) | `/plan-first` 规划执行分离(TDD+选型) | `/plan` 结构化计划 | `/brainstorming` 头脑风暴
...
完整链路（含审查门禁）：
`/init-project` → **审查** → `/refine` → **审查** → `/init-schema` → `/design-spec` → **审查** → `/design-detail` → **审查** → `/design-page` → **审查** → `/data-flow` → **审查** → `/tech-spec` → **审查** → `/align` → `/plan-first` → 编码 → `/code-review` → `/check-page` → 部署

---

## 项目状态

- **版本**: v1.0 | **分支**: main | **阶段**: 编码中（项目骨架已完成，待逐页实现）
